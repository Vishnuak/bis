...
@section('title', __('stop.create'))

@section('content')
<div class="row justify-content-center">
    <div class="col-md-6">
        <div class="card">
            <div class="card-header">{{ __('stop.create') }}</div>
            <form method="POST" action="{{ route('stops.store') }}" accept-charset="UTF-8">
                {{ csrf_field() }}
                <div class="card-body">
                ...
                <div class="card-footer">
                    <input type="submit" value="{{ __('stop.create') }}" class="btn btn-success">
                    <a href="{{ route('stops.index') }}" class="btn btn-link">{{ __('app.cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
